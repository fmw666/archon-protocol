...
| Prior context | `.archon/memos.md`, manifest ADR index, and matching archive keyword headers | Full memo/archive bodies and the relevant ADR ledger (`.archon/decisions.md` for project decisions, the framework ADR ledger at `https://aaep.site/concepts/decisions` for framework decisions); read only matching ADR sections |
...
**Manifest slice scan** (ADR-32 — required when `.archon/manifest/slices/` exists): read the root manifest `## Manifest Slices` index (already hot) and match each slice's path glob against the demand's target / changed-paths candidate set (computed during the radius probe). For each match, load that slice body in full and fold its scoped Concept Glossary / User Language Index / Source Modularity Map rows into the pre-scan context. Skip non-matching slices so per-subtree state stays off the root hot path. Absence of the directory = the root manifest applies everywhere; record `slice_scan: n/a`.
...
- [ ] Governance-docs mirror: yes(framework-docs/<files>)|skip:no-narrative-change|skip:already-mirrored|skip:fast-path|skip:rejected|N/A(no governance-surface change · adopter projects always N/A)
