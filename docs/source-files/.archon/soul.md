...
- **Path-scoped slices**: For large or monorepo projects, project-specifics that apply to one subtree only belong in a path-scoped manifest slice (`.archon/manifest/slices/<slug>.md`, indexed in manifest §Manifest Slices), loaded on demand when a delivery touches that subtree — not on the root manifest hot path. Absence of slices = the root manifest applies everywhere. The slice is the dual of a domain lens: lenses scope reasoning tools, slices scope project state. See ADR-32.
