...
- Cross-file invariant or recurring incident → freeze it as a **structural guard**: a whole-repository static-scan test with a sentinel assertion (scanned count above a floor) and, for existing violations, a grandfather set whose cap equals the current count. A fixed cross-file bug with no guard invites its own return (dev/structural-guard lens tool)
