...
Archon 把文档当作**交付物，而不是事后补充**。Demand 流水线在多个阶段强制执行这一点：
...
3. **漂移检测** — [漂移机制](/architecture/drift) 追踪文档是否反映现实。当代码变更速度超过文档更新，漂移计数器递增。到达阈值时，系统强制对齐："manifest 说认证用的 JWT，但代码已经换成了 session token——更新 manifest。"
