...
- **/docs/specs/**: **Living PRD**. The ONLY source of feature requirements.
- *Action*: Before coding, read the relevant spec. If current code contradicts it, mark code as `Legacy Debt`.
- *Action*: If user changes a requirement, update the spec FIRST, then the code.
- **/docs/arch/**: **System Maps**. Architecture diagrams and module boundaries.
- *Action*: Never import across module boundaries without checking `/arch/boundaries.md`.
- **/docs/rules/**: **Code Constitution**. Strict linting, styling, and pattern rules.
- *Action*: Enforce these rules mercilessly.
...
- Check `/docs/specs/active.md` to confirm the requirement.
...
- If large: Add a checklist item to `/docs/todo/debt_radar.md`.
